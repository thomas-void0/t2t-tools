 ...
 */
type _Parameters<T extends (...arg: any) => any> = T extends (
  ...arg: infer P
) => any
  ? P
  : never;
 ...
 * type example = false
 */
type _IfEquals<A, B, X = A, Y = never> = (<T>() => T extends A
  ? 1
 ...
 * }
 */
type _PickByValue<T, ValueType> = _Pick<
  T,
  {
 ...
 * }
 */
type _PickByValueExact<T, ValueType> = _Pick<
  T,
  {
    [P in keyof T]: [ValueType] extends [T[P]]
 ...
 * }
 */
type _OmitByValue<T, ValueType> = _Pick<
  T,
  {
 ...
 * }
 */
type _OmitByValueExact<T, ValueType> = _Pick<
  T,
  {
 ...
 * type example = string
 */
type _PromiseType<T extends Promise<any>> = T extends Promise<infer P>
  ? P
  : never;
